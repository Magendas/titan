...
airborne.bootstrap.obj.__action = {
	...
	,SELECT_OPTION_VALUES:"SELECT_OPTION_VALUES"
	...
	,is_valid_relation_type:function(relation_type) {
		if(_v.is_not_unsigned_number(relation_type)) {
			return false;
		}
		if(	relation_type === this.ACTION_TABLE_ITEM_RELATION_TYPE_PAIR_TITLE || 
			relation_type === this.ACTION_TABLE_ITEM_RELATION_TYPE_PAIR_TEXT || 
			relation_type === this.ACTION_TABLE_ITEM_RELATION_TYPE_CHAIN_TITLE || 
			relation_type === this.ACTION_TABLE_ITEM_RELATION_TYPE_CHAIN_TEXT ) {
	...
	,get_action_obj_empty:function() {
		...
		var action_obj = {
			...
			,set_action_data:function(action_data_obj, coordinate, action_hierarchy_search_map, action_depth_arr, depth) {
				...
				// wonder.jung
				console.log("set_action_data / 001");
			...
			,get_action_context_obj:function() {
				if(_v.is_valid_str(this.action_context)) {
					return $.parseJSON(this.action_context);
				}
			...
			,set_action_context_attr:function(key, value) {
				...
				if(_v.is_valid_str(this.action_context)) {
					action_context_obj = $.parseJSON(this.action_context);
				}
	...
	,make_element_event_manager:function(event_manager_id, element_jq, action_item_obj, delegate_set_event_manager_prop){
		...
		var element_event_manager = {
			...
			,element_focus_color:null 
			,set_element_focus_color:function(element_focus_color){
				this.element_focus_color = element_focus_color;	
			}
			,get_element_focus_color:function(){
				return this.element_focus_color;	
			}
			,element_focus_bg_color:null 
			,set_element_focus_bg_color:function(element_focus_bg_color){
				this.element_focus_bg_color = element_focus_bg_color;	
			}
			,get_element_focus_bg_color:function(){
				return this.element_focus_bg_color;	
			}
			...
			,move_title_input_group_jq:function(event_type){
			...
			,show_input_mode:function(event_type){
				...
				if( is_select_box ) {
					...
					this.set_event_modal_select_box(event_type);

					// this.show_input_mode_search_n_select();
					// this.set_title_input_jq_search_list_watch_dog_interval();
					// this.set_event_btn_cancel_on_input_group(event_type);
					// this.set_event_select_box_person_name(event_type);

				} else if(_action.EVENT_TYPE_ADD_ROW === event_type){	

					// 새로운 리스트 열을 추가하는 경우
					this.show_input_mode_add_row(event_type);

					this.hide_shy_child();

					console.log("새로운 리스트 열을 추가하는 경우 / event_type ::: ",event_type);

					// set event on input buttons	
					this.set_event_btn_ok_on_input_group(event_type);
					this.set_event_btn_cancel_on_input_group(event_type);
					...
					console.log("그 외의 상황 / event_type ::: ",event_type);

					this.set_event_modal_input_box(event_type);
			...
			,show_input_mode_default:function(event_type){
				...
				this.rollback_row_title_input(event_type);
			...
			,show_input_mode_add_row:function(event_type){
				...
				if(cur_action_item_obj.get_action_is_shy()) {
					...
					var event_type = _obj.EVENT_TYPE_UPDATE_ITEM;
					// var event_type = _obj.EVENT_TYPE_ADD_ROW;
					// var event_type = _obj.EVENT_TYPE_INSERT_ITEM;

					event_manager_after.show_input_mode(event_type);
			...
			,set_colors_reverse:function(){
				...
				var cur_element_focus_color = this.get_element_focus_color();
				var cur_element_focus_bg_color = this.get_element_focus_bg_color();

				// FIX ME
				cur_element_jq.css("color",this.get_element_focus_color());
				this.set_color_background(this.get_element_focus_bg_color());
			...
			,rollback_row_title_input:function(event_type){
				if(this.title_jq == null || this.title_input_jq == null) return;
				...
				if(_action.EVENT_TYPE_ADD_ROW === event_type) {
					this.set_title_input_jq_value("");
				} else {
			...
			,set_event_modal_input_box:function(cur_event_type){

				// 테이블의 여유공간이 없어 modal로 수정사항을 보여줍니다.
				...
				// create modal
				var cur_title_input_jq_value = this.get_title_jq_value();

				console.log("HERE / cur_title_input_jq_value ::: ",cur_title_input_jq_value);
				...
				_action_modal.get(
					...
					, function(modal_obj) {
						...
						console.log("UPDATE / modal_obj ::: ",modal_obj);

						if(modal_obj != null && modal_obj.is_changed == true) {

							var cur_values = modal_obj.get_values();
							
							console.log("HERE / 001");

							var next_value = cur_values.value;
							console.log("UPDATE / next_value ::: ",next_value);

							// 사용자가 변경한 값을 화면에 표시합니다.
							this.set_title_jq_text(
								// new_title
								next_value
								// is_change_attr_input_value
							...
							this.call_delegate_save_n_reload(_obj.ELEMENT_TYPE_INPUT_TEXT, cur_event_type);
						}
				...
				modal_obj.add_textarea_field(
					// title
					"Value"
					// placeholder
					,"Please check the value!"
					// value
					,cur_title_input_jq_value
				);
				modal_obj.show();
			}				
			,set_event_modal_select_box:function(cur_event_type){
				...
				var option_values_list = this.get_action_item_obj().get_action_context_attr(_param.SELECT_OPTION_VALUES);
				var attr = this.get_action_item_obj().get_action_context_attr(_param.ATTR_NAME);

				var option_list = [];
				for (var idx = 0; idx < option_values_list.length; idx++) {
					var option_value = option_values_list[idx];

					var cur_select_option = _obj.get_select_option(option_value,option_value);
					option_list.push(cur_select_option);
				}
				...
				_action_modal.get(
					...
					, function(modal_obj) {
						...
						if(modal_obj != null && modal_obj.is_changed == true) {
							...
							var cur_select_option = _obj.get_select_option(cur_value,cur_value);
							this.call_delegate_save_n_reload(_obj.ELEMENT_TYPE_SEARCH_LIST, _obj.EVENT_TYPE_UPDATE_ITEM, cur_select_option);
			...
			,on_add_btn_click:function(){
				...
				if(cur_action_item_obj.is_table_child_column_list_field_item()) {
					...
					if(cur_action_item_obj.get_action_is_shy()) {
						...
						// 2-1-2. 입력창을 보여줍니다.
						var event_type = _obj.EVENT_TYPE_UPDATE_ITEM;
						this.show_input_mode(event_type);
