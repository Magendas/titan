airborne.bootstrap.view.obj.__action_modal = {
	...
	, get:function(modal_title, _callback, _scope) {
		...
		var modal_obj = {
			...
			, set_event:function() {
				var _self = this;
				this.btn_save_jq.click(function(e) {
					e.stopPropagation();					
					_self.is_changed=true;
					_self._callback.apply(_self._scope,[_self]);
					_self.hide();
				});
				this.modal_jq.on('hidden.bs.modal', function (e) {
					_self.remove();
					_self.is_changed=false;
					// _self._callback.apply(_self._scope,[_self]);
				});

			}
			, hide_btn_save:function() {
				this.btn_save_jq.hide();
			}
			, get_unique_id:function(title) {
				return airborne.bootstrap.view.obj.__action_modal.get_unique_id(title);
			}
			, add_title_fixed:function(title, value) {
				...
				+ "<li id=\"${input_field_id}\" class=\"list-group-item list-group-item-warning\" style=\"padding-left:10px;padding-right:10px;padding-bottom:0px;\">"				
					...
					+ "<form class=\"form-horizontal\" style=\"margin-bottom:10px;\">"
						+ "<div class=\"form-group\" style=\"margin-bottom:0px;\">"
							...
							+ "<div class=\"col-sm-6\">"
								+ "<input type=\"text\" class=\"form-control\" id=\"meeting-theme\" value=\"${value}\" disabled>"
				...
				this.input_list_jq.append(input_field_tag);
				var input_field_jq = this.input_list_jq.find("li#${input_field_id}".replace(/\$\{input_field_id\}/gi, input_field_id));
			...
			, add_textarea_field:function(title, placeholder, value) {

				var textarea_field_id = this.get_unique_id("textarea_field");
				var textarea_field_tag = ""
				+ "<li id=\"${textarea_field_id}\" class=\"list-group-item list-group-item-warning\" style=\"padding-left:10px;padding-right:10px;padding-bottom:0px;\">"				
					.replace(/\$\{textarea_field_id\}/gi, textarea_field_id)
					+ "<form class=\"form-horizontal\" style=\"margin-bottom:10px;\">"
						+ "<div class=\"form-group\" style=\"margin-bottom:0px;\">"

							+ "<label class=\"col-sm-3 control-label\">${title}</label>".replace(/\$\{title\}/gi, title)
							+ "<div class=\"col-sm-8\">"
								+ "<textarea rows=\"5\" cols=\"60\" type=\"text\" class=\"form-control\" id=\"meeting-theme\" placeholder=\"${placeholder}\" style=\"resize:none;\">${value}</textarea>"
									.replace(/\$\{placeholder\}/gi, placeholder)
									.replace(/\$\{value\}/gi, value)
							+ "</div>"
				...
				this.input_list_jq.append(textarea_field_tag);
				var textarea_field_jq = this.input_list_jq.find("li#${textarea_field_id}".replace(/\$\{textarea_field_id\}/gi, textarea_field_id));
				var textarea_jq = textarea_field_jq.find("textarea");
				...
				var field_obj = {
					...
					, clear:function() {
						this.textarea_jq.val("");
					}
				}

				this.field_obj_arr.push(field_obj);
			}
			, add_iframe:function(title, iframe_page_link, iframe_height, is_scrollable) {

				if(_v.isNotValidStr(title)){
					console.log("!Error! / _v.isNotValidStr(title)");
					return;
				}
				if(_v.is_not_valid_str(iframe_page_link)) {
					console.log("!Error! / _v.is_not_valid_str(iframe_page_link)");
					return;
				}
				if(_v.is_not_unsigned_number(iframe_height)) {
					console.log("!Error! / _v.is_not_unsigned_number(iframe_height)");
					return;
				}
				if(is_scrollable == null) {
					is_scrollable = false;
				}

				var iframe_id = this.get_unique_id("iframe");
				var iframe_tag = ""
				+ "<li id=\"${select_box_id}\" class=\"list-group-item list-group-item-warning\" style=\"padding-left:10px;padding-right:10px;padding-bottom:0px;\">"
					+ "<iframe id=\"<_v>\" src=\"<LINK>\" width=\"100%\" height=\"<HEIGHT>px\" frameborder=\"0\" scrolling=\"<SCROLL>\"></iframe>"
					.replace(/\<LINK\>/gi, iframe_page_link)
					.replace(/\<_v\>/gi, iframe_id)
					.replace(/\<HEIGHT\>/gi, iframe_height)
					.replace(/\<SCROLL\>/gi, (is_scrollable)?PROPS.PARAM_SET.YES:_param.NO)
				+ "</li>"

				this.input_list_jq.append(iframe_tag);

				// no event
				// wonder.jung

			}			
			, add_select_box:function(title, option_list, selected_value) {

				var select_box_id = this.get_unique_id("select_box");
				var select_box_tag = ""
				+ "<li id=\"${select_box_id}\" class=\"list-group-item list-group-item-warning\" style=\"padding-left:10px;padding-right:10px;padding-bottom:0px;\">"				
					.replace(/\$\{select_box_id\}/gi, select_box_id)
					+ "<form class=\"form-horizontal\" style=\"margin-bottom:10px;\">"
						+ "<div class=\"form-group\" style=\"margin-bottom:0px;\">"
							...
							+ "<div class=\"col-sm-8\">"
								...
								+ "<select class=\"form-control\" name=\"list_search_tab\">"							
				;
				...
				for(var idx=0;idx<option_list.length;idx++) {
					...
					var option = option_list[idx];
					if(_obj.is_not_valid_search_option(option)) {
						continue;
					}

					var selected = "";
					var value = option.get_value();
					var key = option.get_key();
				...
				this.input_list_jq.append(select_box_tag);
				var container_jq = this.input_list_jq.find("li#${select_box_id}".replace(/\$\{select_box_id\}/gi, select_box_id));
				var select_box_jq = container_jq.find("select");

				var field_obj = {
					title:title
					, select_box_jq:select_box_jq
					, selected_key:null
					, set_selected_option_key:function(key) {
						this.selected_key = key;
					}
					, get_option_key:function() {
						return this.selected_key;
					}
					, get_selected_option:function() {
						return {key:this.get_option_key(), value:this.get_value()};
					}
					, get_value:function() {
						var selected_value = this.select_box_jq.val();
						return this.select_box_jq.val();
					}
					, get_title:function() {
						return this.title;
					}
					, focus:function() {
						this.select_box_jq.focus();
					}
				}

				// 선택된 값을 세팅합니다.
				var selected_key = select_box_jq.find(":selected").text();
				field_obj.set_selected_option_key(selected_key);

				select_box_jq.change(function(e){
					var _self_jq = $(this);
					var selected_key = _self_jq.find(":selected").text();
					field_obj.set_selected_option_key(selected_key);
				});
			...
			, get_values:function() {
				...
				for(var idx=0;idx < this.field_obj_arr.length;idx++) {
					...
					if(field_obj.get_selected_option != null) {
						values[key] = field_obj.get_selected_option();						
					} else {
