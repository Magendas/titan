airborne.dates = {
	...
	*/
	...
	// @ desc : millsec 단위 시간을 재주는 타이머 객체를 줍니다.
	,get_timer_in_seconds:function() {
		...
		var timer = {
			...
			, time_elapsed:null
			, start:function() {
				this.time_elapsed_obj = new Date();
			}
			, stop:function() {
				var now = new Date();
				this.time_elapsed = now - this.time_elapsed_obj;
			}
			, get_time_elasped:function() {
				var cur_time_elasped = Math.round(this.time_elapsed/10) / 100;
				if(this.quiz_time_limit < cur_time_elasped) {
					return (this.quiz_time_limit * 100) / 100;
				}
				return Math.round(this.time_elapsed/10) / 100;
			}
		}	
		
		return timer;
	}
	// @ desc : 초 단위 시간을 재주는 타이머 객체를 줍니다.
	,get_timer_in_seconds:function(total_time_sec) {
		...
		var timer = {
			...
			, get_time_left:function() {
				...
				if(this.time_left_sec == 0) {
					return 0;
				}

				return --this.time_left_sec;
			}
			, callback_show_time:null
			, callback_show_time_scope:null
			, show_time:function() {

				if(this.callback_show_time == null) {
					return;
				}
				if(this.callback_show_time == null) {
					return;
				...
				// 콜백 함수로 화면에 남은 시간을 그립니다.
				this.callback_show_time.apply(this.callback_show_time_scope,[this]);
		...
		var timer_interval = setInterval(function() {
			...
			var cur_time_left = timer.get_time_left();
		...
		timer.timer_interval = timer_interval;
