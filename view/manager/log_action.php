<?php
	...
	// PAGINATION - INIT
	$action_total_row_cnt = $mysql_interface->select_action_log_total_cnt($PROPS);
	$preprocessor->set_pagination_total_cnt($action_total_row_cnt);
	$PROPS = $preprocessor->get_props();
	// PAGINATION - DONE
	...
	$action_log_list = 
	$mysql_interface->select_action_log($PROPS);
	$PROPS->action_log_list = $action_log_list;
...
<body role="document">
	...
	array(
		...
		, "[__ACTIVE_ACTION_LOG__]"=>"active"
		, "[__ACTIVE_ACTION_LOG_STYLE__]"=>"color:#FFFFFF !important;"
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main" style="margin-top:15px;">			
		...
		<?php
			...
			foreach($action_log_list as $action_log) {
				...
				$__id = intval($action_log->__id);
				$__browser = $action_log->__browser;
				$__ip = $action_log->__ip;
				$__os = $action_log->__os;
				$__user_name = $action_log->__user_name;
				$__type = $action_log->__type;
				$__msg = $action_log->__msg;
				$__time_start = $action_log->__time_start;
				...
				echo 		"<td id=\"__info\"><small>$__os / $__browser / $__ip<br/>$__type<br/>$__user_name<br/>$__time_start</small></td>";
